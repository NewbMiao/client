...
const RETRY_TIMES_RANGE: RangeInclusive<u8> = 0..=3;
...
impl Proxy {
    ...
    pub async fn new(
        ...
        seed_peers_health_check_interval: Option<Duration>,
        ...
        let seed_peer_selector = Arc::new(
            SeedPeerSelector::new(scheduler_client, seed_peers_health_check_interval).await?,
        );
...
impl Proxy {
    ...
    async fn create_client(&self, request: &GetRequest) -> Result<Client> {
        ...
        for peer in seed_peers.iter() {
            ...
            debug!("try to get request client via proxy server: {}", proxy_addr);
            match self.client_pool.get_or_create(&proxy_addr) {
                Ok(client) => return Ok(client),
                Err(err) => {
                    error!(
                        "failed to build client with proxy server {}: {}",
                        proxy_addr, err
                    );
                    continue;
                }
...
mod tests {
    ...
    async fn test_proxy_new_invalid_retry_times() -> Result<()> {
        ...
        let scheduler_endpoint = format!("http://0.0.0.0:{}", mock_server.port().unwrap());
        let result = Proxy::new(scheduler_endpoint, Some(Duration::from_secs(60)), Some(5)).await;
    ...
    async fn test_get_request_success() {
        ...
        let proxy = create_test_proxy().await.unwrap();
    ...
    async fn test_get_dfdaemon_error() {
        ...
        if let Err(Error::DfdaemonError(err)) = result {
            ...
            assert!(false, "expected DfdaemonError");
        }
    ...
    async fn test_response_stream_reading() {
        ...
        if let Some(mut reader) = response.reader {
            ...
            assert!(false, "expected reader to be present");
        }
    ...
    async fn create_test_proxy() -> Result<Proxy> {
        let mock_server = setup_mock_scheduler().await?;

        let scheduler_endpoint = format!("http://0.0.0.0:{}", mock_server.port().unwrap());
        let proxy = Proxy::new(scheduler_endpoint, None, None).await.unwrap();
